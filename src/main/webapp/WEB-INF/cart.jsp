...
<%@page import="entite.PanierContainer"%>
...
<html>
    ...
    <body>
       ...
       <ul>
            ...
            <%
            HashMap<String, PanierContainer> chaqueProduitDsPanier = PanierManager.getPanier(SessionManager.getSession(request, "panier"));
            ...
            <div class="card_header">
                ...
                <i style="font-size:50px; color:#333;" class="fa">&#xf07a;</i> <span>Mon panier</span>
                    
                    <%
                        if(chaqueProduitDsPanier != null){
                         %>
                            <span class="items"> <%= chaqueProduitDsPanier.size() %> <%=  (chaqueProduitDsPanier.size() <= 1 ? "Article" : "Articles") %></span>
                         <%
                        }else{ 
                         %>
                             <span class="items">0 Article</span>
                         <%
                          }
                        ...
                        <%
                            ...
                            DecimalFormat dec = new DecimalFormat("#0.00");
                               ...
                               if(chaqueProduitDsPanier != null){
                                   ...
                                   for (HashMap.Entry<String, PanierContainer> entry : chaqueProduitDsPanier.entrySet()) {
                                 PanierContainer value = entry.getValue();
                                 Produit produit = value.getProduit();
                                 int qt_de_chaque_produit = value.getQuantite();
                                 prix_total += produit.getPrix() * value.getQuantite();
                                 double prixCourant = produit.getPrix() * value.getQuantite();
                                %>
 
                <article class="cart" >
                    
                    <img src="<%= produit.getUrlImage() %>"/>
                    <div class="product_description">
                        <form action="panierControleur" method="post">
                         <select name="prd_qt_selected">
                                <%
                                    for(int i = 0; i < produit.getQuantite(); i++){
                                        if((i + 1) == qt_de_chaque_produit){
                                        %>
                         ...
                         <input type="hidden" name="upt_prd" value="<%= produit.getId() %>">
                         <input type="submit" value="Modifier" class="message_submit" style="padding:11px;">  
                        </form>
                        <label ><%= produit.getNom() %><label> <br />
                           <label style="margin-top:-20px !important;" ><%= dec.format(prixCourant) %> $ CAD</label>
                           <form action="panierControleur" method="post">
                               <input type="hidden" name="rmv" value="<%= produit.getId() %>">
                               <button style="font-size:24px; float:right; right:0px; cursor:pointer; background-color:white; border:none;"><i class="fa">&#xf014;</i></button>
                           </form>
                    </div>
                            ...
                            <% if(chaqueProduitDsPanier != null){ 
                                ...
                                if(chaqueProduitDsPanier.size() > 0){ %>
                  <div id="passer_la_caisse">
                      <form action="checkoutControleur" method="post" style="height:50px;">
                        <input type="hidden" name="prixPanier" value="<%= dec.format(prix_total) %>">
                    <button><%= dec.format(prix_total) %> $ CAD - Passer la caisse</button> 
                      </form>
                  </div>
                    </div>
                    <%
                        ...
                        <div id="panier_vide">
                       <h2>Votre panier est pour l'instant vide.</h2>
                        </div>
                       <%
                     }
